defmodule CustomSet do
  ...
  def subset?(custom_set_1, custom_set_2) do
    custom_set_1.map |> Map.keys() |> Enum.all?(fn k -> contains?(custom_set_2, k) end)
  end

  @spec disjoint?(t, t) :: boolean
  def disjoint?(custom_set_1, custom_set_2) do
    custom_set_2.map |> Map.keys() |> Enum.all?(fn k -> not contains?(custom_set_1, k) end)
  end
  ...
  def intersection(custom_set_1, custom_set_2) do
    custom_set_1.map |> Map.keys() |> Enum.filter(fn k -> contains?(custom_set_2, k) end) |> new()
  end

  @spec difference(t, t) :: t
  def difference(custom_set_1, custom_set_2) do
    custom_set_1.map |> Map.keys() |> Enum.reject(fn k -> contains?(custom_set_2, k) end) |> new()
  end

  @spec union(t, t) :: t
  def union(custom_set_1, custom_set_2) do
    Map.merge(custom_set_1.map, custom_set_2.map) |> Map.keys() |> new
  end
end
